...
extension Alert {
    var modal: UIAlertController {
        ...
        case .juiceSucess(let juice):
            let alretController = UIAlertController(title: "알림", message: "\(juice.name) 나왔습니다! 맛있게 드세요!", preferredStyle: .alert)
            let yes = UIAlertAction(title: "확인", style: .default, handler: nil)
            alretController.addAction(yes)
            return alretController
        case .juiceFail(let handler):
            let alretController = UIAlertController(title: "알림", message: "재료가 모자라요. 재고를 수정할까요?", preferredStyle: .alert)
